...
from .retrieval import build_search_query, compact_record, normalize_data_source_url, rank_rows
...
class MiseMemoryProvider(MemoryProvider):
    ...
    def _resolve_config(self, hermes_home: str = "") -> str:
        home = Path(hermes_home or self.hermes_home or os.environ.get("HERMES_HOME") or Path.home() / ".hermes")
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        self.hermes_home = str(kwargs.get("hermes_home") or os.environ.get("HERMES_HOME") or Path.home() / ".hermes")
        try:
            ...
            if self.client and self.data_source_url:
                result = self.client.query(f'SELECT "Memory Key" FROM "{self.data_source_url}" LIMIT 1', [])
    ...
    def _search(self, query: str, limit: int = 10):
        sql, params = build_search_query(query, limit=limit, data_source_url=self.data_source_url)
        rows = self.client.query(sql, params).get("results", [])
